...
require "/home/cs/dbinfo/dbinfo2018.txt";
...
$sql = "SELECT programs.studentID,rishusha.name,kai,kihon_hatten,bangou,status,comment";
$sql .= " from programs,TA,rishusha";
$sql .= " where TA.han=rishusha.han and programs.studentID=rishusha.studentID";
$sql .= " and TA.TAID='$TAID'";
$sql .= " order by status, kihon_hatten, kai, bangou, programs.studentID;";
$sth = $dbh->prepare($sql);
$sth->execute();
$error_message = $sth->errstr;
if ($error_message) {
...
print "</br>";
print "<a href=\"./download_kakunin_all.cgi?han=$han\">TA確認待ちの課題全部一括ダウンロード</a>";
...
print "<td>";print "コメント入力欄";print "</td>";
print "<td>";print "コメント";print "</td>";
print "</tr>";

while (my $ary_ref = $sth->fetchrow_arrayref) {
    my ($studentID, $name, $kai, $kihon_hatten, $bangou, $status, $comment) = @$ary_ref; # @をつけて配列にする
    # $ary_refは配列へのレファレンス
    print "<tr>";
    ...
    print "</td>";
    print "<td>"
	. "<form action=\"commentsend.cgi\" method=\"post\" target=\"iframe\">"
	. "<input type=\"hidden\" name=\"studentID\" value=\"$studentID\">"
	. "<input type=\"hidden\" name=\"kai\" value=\"$kai\">"
	. "<input type=\"hidden\" name=\"kihon_hatten\" value=\"$kihon_hatten\">"
	. "<input type=\"hidden\" name=\"bangou\" value=\"$bangou\">"
	. "<input name=\"comment\" maxlength=\"1000\" size=\"18\" type=\"text\">"
	. "<input value=\"送信\" onclick=\"submit()\" type=\"button\">"
	. "</form>"
	. "<iframe name=\"iframe\" style=\"width:0px;height:0px;border:0px;\"></iframe>"
	. "</td>";
    print "<td>" . $comment . "</td>";
}
print "</table>";
