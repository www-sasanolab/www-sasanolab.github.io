;;Time-stamp: <Thu Jan 28 15:00:07 東京 (標準時) 2010>

;;lambda-parse.el version 0.02 
;;
...
(defvar toggle-message nil)

;;マクロは実行中に上書きしても大丈夫らしい
...
(defun instanciate-type-clone (type)
  (let ((free-variables (cdr (assoc 'tyvar-list type)))
    ...
    (my-message (format "■■cloned %s" (type-to-string result)))
    result))
...
(defun inference- (env expr)
  ...
  (let ((klass (cdr (assoc 'class expr)))
	...
	(setq type-variables-in-bindType (let ((result (list)))
					   (enumerate-all-type-variable-tree (cdr (assoc 'tree bindType)))
					   result))
	(my-message (format "■多相型化 (bindType:%s)" (type-to-string bindType)))
	(my-message (format "  ■環境に含まれる型変数: %s" type-variables-in-env))
	(my-message (format "  ■bindTypeに含まれる型変数: %s" type-variables-in-bindType))
	
	(while (car type-variables-in-bindType)
	  (my-message (format "    ▼比較: in bindType: %s , in Env: %s => %s (eq: %s )" 
			      (car type-variables-in-bindType) 
			      type-variables-in-env
			      (if (member (car type-variables-in-bindType) type-variables-in-env)
				  "単相" "多相")
			      (eq (car type-variables-in-bindType) 
				  (car (member (car type-variables-in-bindType) type-variables-in-env)))))
	  (if (not (member (car type-variables-in-bindType) type-variables-in-env))
	      (setq generic-type-variables (cons (car type-variables-in-bindType)
						 generic-type-variables)))
	  (setq type-variables-in-bindType (cdr type-variables-in-bindType)))

	(my-message (format "  ■bindTypeのgenericな型変数: %s" generic-type-variables))
	
	(setcdr (assoc 'tyvar-list bindType) generic-type-variables)

	;;多層型に
	(if generic-type-variables
	    (setcdr (assoc 'generic bindType) t))
	(my-message (format "bindType(%S) generic is %s (tyvar:%s)" 
			 (type-to-string bindType)
			 (cdr (assoc 'generic bindType))
			 (inference-get-tyvar-list bindType)))
	
	(setcdr (assoc 'bind-inferred expr) (clone-type bindType))
	(setcdr (assoc 'bind-inferred2 expr) bindType)
  ...
  (let ((klass (cdr (assoc 'class expr)))
    ...
    (cond
     ...
     ((eq klass 'let)
      ...
      (my-message (format "%s   bindName: %s {%s} => {%s}" sp (cdr (assoc 'bindName expr))
		       (type-to-string (cdr (assoc 'bind-inferred expr)))
		       (type-to-string (cdr (assoc 'bind-inferred2 expr)))))
