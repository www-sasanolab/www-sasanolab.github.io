...
void drawbackground(GLubyte *bits) {
  ...
  //  glDrawPixels(1920, 1080, GL_RGB, GL_UNSIGNED_BYTE, bits);
    glDrawPixels(2560, 1440, GL_RGB, GL_UNSIGNED_BYTE, bits);
}
...
unsigned __stdcall disp (void *arg) {
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
  readBits(bits, "sit2.bmp", WIDTH, HEIGHT); /* 2つ目の画像読み込み */
  glTexImage2D(GL_TEXTURE_2D, 0, 3, WIDTH, HEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, bits);
  /* ここまで */
  
  if ((bits = realloc (bits, 2560*1440*3))==NULL) { /*  背景画像読み込み用領域確保 */
    fprintf (stderr, "allocation failed.\n");
    exit(0);
  }
  readBits(bits, "skytree2560_1440.bmp", 2560, 1440);
