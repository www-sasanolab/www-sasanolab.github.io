...
void display (char * ssd, GLuint textureID) 
{
  ...
  glPushMatrix(); // 現在の行列（model view変換行列）をstackにpushして保存
  glTranslatef(locx-20.0, locy-20.0, 0.0); // 図形の位置指定
  glRotatef((double) r, 0.0, 0.0, 1.0); // z軸の周りにr度回転

  glEnable(GL_TEXTURE_2D);
  glBindTexture(GL_TEXTURE_2D, textureID);

  glBegin(GL_POLYGON); // 中心（回転軸）が(0,0)になるように図形描画
  glTexCoord2f(0, 0); glVertex2f(-10.0, -10.0);
  glTexCoord2f(0, 1); glVertex2f(-10.0, 10.0);
  glTexCoord2f(1, 1); glVertex2f(10.0, 10.0);
  glTexCoord2f(1, 0); glVertex2f(10.0, -10.0);
  glEnd();

  glPopMatrix(); // 保存しておいた行列（model view変換行列）をstackからpop

  r++; // 回転角度を1度プラス
  if(r==360)r=0; // 360度になったら0度に戻す

  glEnable(GL_TEXTURE_2D);
  glBindTexture(GL_TEXTURE_2D, textureID);
  glBegin(GL_POLYGON); // もう一枚の画像は回転させずに表示（前のプログラムと同じ）
