...
void display (char * ssd, GLUquadricObj *sphere) 
{
  ...
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);  // DEPTH_BUFFER_BITもクリアする

  glPushMatrix();
  glTranslatef(locx-20.0, locy-20.0, 0.0); // 図形の描画位置指定
  glRotatef(angle, 0.5 , 1 , 0.25);// 図形の回転軸と回転角度指定
  angle=angle+1;// 次回のdisplay関数呼び出し時のために角度を1増加

  // 球の描画（テクスチャ付き）
  glEnable(GL_TEXTURE_2D); // テクスチャを張り付ける
  gluSphere(sphere,20.0, 50, 50); // 球の描画
...
unsigned __stdcall disp (void *arg) {
  ...
  glEnable(GL_DEPTH_TEST); // 一番手前に描画されたものが見えるように（球の反対側が見えないように）

  // 球に張り付けるオブジェクトの設定
  sphere = gluNewQuadric(); // 球に張り付けるオブジェクトト生成
  gluQuadricTexture(sphere,GLU_TRUE); // テクスチャを使用するように設定
  gluQuadricDrawStyle(sphere, GLU_FILL); // 描画スタイルの設定
  // gluQuadricDrawStyle(sphere, GLU_LINE); // 描画スタイルの設定
  // gluQuadricDrawStyle(sphere, GLU_SILHOUETTE); // 描画スタイルの設定
  // gluQuadricDrawStyle(sphere, GLU_POINT); // 描画スタイルの設定
  
  /* 表示関数呼び出しの無限ループ */  
  while(1) {
    Sleep(15); // 0.015秒待つ
    display(ssd, sphere); // 描画関数呼び出し
    glFlush(); // 画面描画強制
    SwapBuffers(hDC); // front bufferとback bufferの入れ替え
    if (finish == 1){ // finishが1なら描画スレッドを終了させる
      gluDeleteQuadric(sphere); // 球に張り付けるオブジェクトの解放
      return 0;
    }
  }
}
