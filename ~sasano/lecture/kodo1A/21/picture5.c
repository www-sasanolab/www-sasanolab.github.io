...
void display (char * ssd) 
{
  ...
  glBegin(GL_POLYGON); // 多角形を描画
  // glTexCoord2f(s,t): texture内のs座標（横）、t座標（縦）を指定。
  // Texはtexture、Coordはcoordinate（座標）の頭文字。
  // 2はs, tの2つを指定という意味で、fは浮動小数点（floating point）。
  // 基本は0から1の範囲だが、この範囲をはみ出てもよい。
  // はみ出た場合は、繰り返すかどうかをglTexParameteriの
  // 第三引数に指定。GL_CLAMPは繰り返さない、GL_REPEATは繰り返す。
  // 第二引数がGL_TEXTURE_WRAP_Sの場合はs方向の繰り返し、
  // 第二引数がGL_TEXTURE_WRAP_Tの場合はt方向の繰り返しを指定。
  // glTexCoord2fでテクスチャ内の座標を指定した直後に
  // glVertex2fで描画する座標を指定し、これを多角形の辺の順番に繰り返す。
  // 辺の順番はどこから始めてもよく、回る方向も逆回りでもOKだが、
  // 順番を適当にすると多角形にならなくなる。
  glTexCoord2f(0.15, 0); glVertex2f(locx-17.0, locy-20.0);
  glTexCoord2f(0.85, 0); glVertex2f(locx-3, locy-20.0);
  glTexCoord2f(1, 0.7); glVertex2f(locx, locy-6);
  glTexCoord2f(0.5, 1); glVertex2f(locx-10.0, locy);
  glTexCoord2f(0, 0.7); glVertex2f(locx-20.0, locy-6);
  glEnd();
